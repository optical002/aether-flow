package aetherflow.engine.os.executors

import aetherflow.engine.core.logger.ASyncLogger
import aetherflow.engine.os.ShellExecutor
import aetherflow.engine.os.powershell.PowershellScript
import zio.*
import zio.stream.*
...
object PowershellExecutor extends ShellExecutor[PowershellScript] {
  ...
  def readNewLines(startAt: Int, filePath: Path, logger: ASyncLogger): Task[(List[String], Int)] = for {
  ...
  def streamFile(
    filePath: Path, logger: ASyncLogger
  ...
  def program(
    script: String, logger: ASyncLogger
  ): Task[Seq[String]] = for {
  ...
  override def process(
    script: PowershellScript, logger: ASyncLogger
